...
class DataGenerator:
    ...
    def generate_system_prompt(self) -> str:
        ...
        Key insight from MVP: Shorter prompts work better for SLMs.
        They learn from examples, not verbose instructions.
        """
        tool_list = ", ".join([t.name for t in self.config.tools])
        ...
        prompt = f"""You are an assistant that helps users with: {self.config.problem_statement}
...
Generate exactly {count} examples. Output ONLY the JSON array, no explanation."""
        ...
        try:
            ...
            print(f"⚠️ Failed to parse response for {tool.name}: {e}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate training data for SLM fine-tuning",
...
Examples:
  ...
  python data_generator.py --problem "calendar agent" --tools tools.json --yes
    ...
    parser.add_argument(
        "--problem", "-p",
        required=True,
        help="Problem statement (e.g., 'I need a calendar agent')"
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help="LLM provider (auto-detected from API key if not specified)"
    )
    ...
    if not args.api_key:
        ...
        print("   Set OPENAI_API_KEY or ANTHROPIC_API_KEY env var, or use --api-key")
        return 1
